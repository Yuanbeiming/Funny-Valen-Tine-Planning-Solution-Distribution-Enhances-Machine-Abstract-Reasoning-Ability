...
import model_reverse as model_vit
import argparse
...
# bd_train = SV(args.root_path, type='bd_train')
# bd_train_loader = DataLoader(bd_train, batch_size=args.batch_size, shuffle=True, num_workers=8, prefetch_factor=4,pin_memory=True)
...
# train_loader, num_train, data_name = bd_train_loader, len(bd_train), "BA "
...
# ff_train = SV(args.root_path, type='ff_train')
# ff_train_loader = DataLoader(ff_train, batch_size=args.batch_size, shuffle=True, num_workers=8, prefetch_factor=4,pin_memory=True)
...
# train_loader, num_train, data_name = ff_train_loader, len(ff_train),"FF "

# val_loader,num_val,data_name_1 = FF_test_loader,len(FF_test),"FF "
...
hd_train = SV(args.root_path, type='hd_train')
hd_train_loader = DataLoader(hd_train, batch_size=args.batch_size, shuffle=True, num_workers=8, prefetch_factor=4,pin_memory=True)
...
train_loader, num_train, data_name, HD = hd_train_loader, len(hd_train), "HD ", True

val_loader,num_val,data_name_1 = NV_test_loader,len(NV_test),"NV "
...
val_loader_1,num_val_1,data_name_2 = CM_test_loader,len(CM_test),"CM "
#%%
...
#model.load_state_dict(torch.load('/home/user/ybm/RAVEN10000/model_Clip_raven_NA_6000__now.pt', map_location = 'cpu'))
...
#optimiser.load_state_dict(torch.load('/home/user/ybm/RAVEN10000/optimiser_Clip_raven_NA_6000__now.pt'))
...
while epoch < num_epoch:
    ...
    loss_test_2 = 0
    # loss_test_all = 0
    with tqdm(total=len(train_loader)  + len(val_loader) + len(val_loader_1) if HD else len(train_loader)  + len(val_loader)) as pbar:
    ...
    print(("epoch:{}\t loss_train:{:.4f}\t " + data_name + "choose_accuracy_train:{:.4f}\t loss_val:{:.4f}\t " + data_name_1 + "choose_accuracy_val:{:.4f}\t  loss_val:{:.4f}\t " + data_name_2 + "choose_accuracy_val:{:.4f}\t  learning_rate:{:.8f}").\
          format(epoch, loss_train,  accuracy[1], loss_test, accuracy_val[0], loss_test_1, accuracy_val[1],  lr_decay.get_lr()[0]))

    with open("test_on_"+text_name+".txt", "a") as f:  # 打开文件
        
        f.write(("epoch:{}\t loss_train:{:.4f}\t " + data_name + "choose_accuracy_train:{:.4f}\t loss_val:{:.4f}\t " + data_name_1 + "choose_accuracy_val:{:.4f}\t  loss_val:{:.4f}\t " + data_name_2 + "choose_accuracy_val:{:.4f}\t  learning_rate:{:.8f}").\
              format(epoch, loss_train,  accuracy[1], loss_test, accuracy_val[0], loss_test_1, accuracy_val[1],  lr_decay.get_lr()[0]))
    epoch += 1
