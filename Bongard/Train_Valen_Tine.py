...
optimiser_G = torch.optim.AdamW(model.module.generate.parameters(), lr=1e-5, weight_decay = args.weight_decay)

optimiser_D = torch.optim.AdamW(model.module.discrim.parameters(), lr=3e-5, weight_decay = args.weight_decay)
    ...
    with tqdm(total=len(train_loader)  + len(val_loader) + len(val_loader_1) + len(val_loader_2) + len(val_loader_3)) as pbar:
        ...
        for _, x_train, _ in train_loader:
            ...
            if epoch%num_D_iter == 0:
                ...
                loss = model.module.loss_function_D(out_train) 
                model.zero_grad()
                loss.backward()#回传
                optimiser_D.step()  
            ...
            if epoch%num_G_iter == 0:
                ...
                loss = model.module.loss_function_G(out_train) 
                model.zero_grad()
                loss.backward()#回传
                optimiser_G.step()  
