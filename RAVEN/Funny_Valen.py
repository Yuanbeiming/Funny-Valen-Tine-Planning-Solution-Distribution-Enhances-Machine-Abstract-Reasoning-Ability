...
class raven_clip(nn.Module):
    ...
    def forward(self, x):
        ...
        return *list(map(lambda t: t[:,:,:,0].mean(dim = (1,2)), qkv)), x.reshape(-1, self.low_dim), sum(list(out)), bias.reshape(-1, self.w, self.low_dim), state, recon_up, recon_down, recon_bias_up, recon_bias_down
    #"""
    ...
    def loss_function(self, *out, target_shape, target_line, idx):
        ...
        # idx = None
        x_shape, x_line, z, x, bias, state, recon_up, recon_down, recon_bias_up, recon_bias_down = out
