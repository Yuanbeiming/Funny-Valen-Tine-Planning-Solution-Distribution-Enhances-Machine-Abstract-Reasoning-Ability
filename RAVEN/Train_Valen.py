...
import clip_perfect_kv_q_cov_doctor_embedding_space as model_vit 
...
t =t_train= transforms.Resize((80,80))
...
#params = torch.load('model_Clip_raven_perfect_cross_cov_doctor_embedding_space_155000_in_distribute_four_out_center_single_now_.pt', map_location = 'cpu')
params = torch.load('model_Clip_raven_perfect_cross_cov_doctor_embedding_space_155000_distribute_nine_now_.pt', map_location = 'cpu')
#params = torch.load('./model_'+name+'_now.pt', map_location = 'cpu')
"""

for k, q in model.named_parameters():

    if k[:3] == 'vit' or k[:len('g_function')] == 'g_function': 
...
#optimiser.load_state_dict(torch.load('/home/user/ybm/pgm/optimiser_Clip_pgm_perfect_cross_cov_master_opposite_II_1200000_neutral_best_99_01.pt'))
...
while epoch < num_epoch:
    ...
    with tqdm(total=len(train_loader)  + len(val_loader)) as pbar:
        ...
        for x_train, label,  label_in, idx in train_loader:
            ...
            x_train = t_train(x_train).float().to(device)
    ...
    if dist.get_rank() == 0:
        print("epoch:{}\t loss_train:{:.4f}\n accuracy_train: out:{:.4f}\t in:{:.4f}\t  choose:{:.4f}\n  accuracy_val: out:{:.4f}\t in:{:.4f}\t choose:{:.4f}\n learning_rate:{:.8f}\n".format(epoch, loss_train,  *accuracy[:3],
