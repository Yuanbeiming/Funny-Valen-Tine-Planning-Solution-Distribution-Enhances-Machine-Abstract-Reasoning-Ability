...
class Parallel_Attention(nn.Module):
    def __init__(self, words, dim, heads = 8, dim_head = 32, dropout = 0.):
        #dim_head: dim of signle head
        super().__init__()
        inner_dim = dim_head *  heads
        project_out = not (heads == 1 and dim_head == dim)
        ...
        self.k_encoding = nn.Parameter(torch.randn(1,
                                                words,       
                                                heads,       
                                                inner_dim) )     
        
        self.v_encoding = nn.Parameter(torch.randn(1,
                                                words,       
                                                heads,       
                                                inner_dim) )    
    ...
    def forward(self, x, **kwargs):
        ...
        k = k.repeat(1,1,1,h) + self.k_encoding
        
        v = v.repeat(1,1,1,h) + self.v_encoding
        
        q = rearrange(q, 'b n h d -> b h n d', h = h)
        
        k = rearrange(k, 'b n h (H d) -> b H (n h) d', h = h, H = h) 
        
        v = rearrange(v, 'b n h (H d) -> b H (n h) d', h = h, H = h) 
        
        
        dots = einsum('b h i d, b h j d -> b h i j', q, k) 
        
        
        dots = rearrange(dots, 'b h i j -> b i h j')
        
        dots_weight = F.softmax(dots, dim = -2).sum(dim = -1, keepdim = True)
        
        dots = dots/dots_weight
        
        if self.training:
            
            dots_select = F.gumbel_softmax(dots.data, dim = -2, hard = True).eq(0)
            
            
        else:
        
            dots_select = dots.ne(dots.data.max(dim = -2, keepdim = True)[0])
           
        dots.masked_fill_(dots_select, -1e9)

        attn = self.attend(dots* self.scale) + 1e-9

 
        attn = rearrange(attn, 'b i h j -> b h i j')
        out = einsum('b h i j, b h j d -> b h i d', attn, v)
        out = rearrange(out, 'b h n d -> b n (h d)')
        return self.to_out(out)  
...
class Routing(nn.Module):
    def __init__(self, iterations, words, dim, heads = 8, dim_head = 32, alpha = 1):
        ...
        self.W = nn.utils.spectral_norm(nn.Parameter(.001*torch.randn(1,
                                                self.heads,   #6x6x32  
                                                words,       #16
                                                inner_dim,       #256
                                                dim_head) )     #32
...
class Routing_norm_Transformer_layer(nn.Module):
    def __init__(self, iterations, words, dim, depth, heads, dim_head, mlp_dim, alpha, dropout = 0.):
        super().__init__()
        #self.param = nn.Parameter(1e-8*torch.ones(depth))  
        self.param = torch.ones(depth)#32
        ...
        for _ in range(depth):#L
            self.layers.append(nn.ModuleList([
                ...
                PreNorm(dim, Routing_norm(iterations = iterations, words = words, dim = dim, heads = heads, dim_head = dim_head, alpha = alpha)),
                PreNorm(dim, FeedForward(dim, mlp_dim, dropout = dropout)),
            ]))
...
class Routing_norm_Transformer(nn.Module):
    def __init__(self, iterations, words, dim, depth, heads, dim_head, mlp_dim, alpha, dropout = 0.):
        super().__init__()
        self.pos_embedding = nn.Parameter(torch.randn(1, words, dim))
        self.transformer = Routing_norm_Transformer_layer(iterations, words, dim, depth, heads, dim_head, mlp_dim, alpha, dropout = dropout)
        self.dropout = nn.Dropout(dropout)
...
class Parallel_Routing_Attention(nn.Module):
    def __init__(self, iterations, words, dim, heads = 8, dim_head = 32, dropout = 0.):
        #dim_head: dim of signle head
        super().__init__()
        inner_dim = dim_head *  heads
        project_out = not (heads == 1 and dim_head == dim)
        ...
        self.W = nn.Parameter(torch.randn(1,
                                                words*self.heads,   #6x6x32  
                                                words*self.heads*self.heads,       #16
                                                dim_head) )     #32
    ...
    def routing(self, u_hat):
        b = torch.zeros_like( u_hat )#同尺寸可以内积相乘 torch.Size([256, 1152, 10, 16])
        u_hat_routing = u_hat.data #前两次禁止回传
        for i in range(self.iterations):#3次迭代
            c = F.softmax(b, dim=2)   #在第三维度上进行softmax,10类连接强度，初始值1/10 torch.Size([256, 1152, 10, 16]
            if i==(self.iterations-1):#最后一次迭代保存梯度
                s = (c*u_hat).sum(1, keepdim=True)  #tor, input_size = 201ch.Size([256, 1, 10, 16]
    ...
    def grad_mask(self, x):
        
        mask = x.eq(0)
        
        x_ = torch.zeros_like(x)
        
        x_.masked_fill_(mask, -1e9)
    ...
    def forward(self, x, **kwargs):
        ...
        v = rearrange(v, 'b n h d -> b () (n h) d', h = h).expand(-1,h,-1,-1)
        
        weight = k*self.W #b, nh, nhh, d
        
        weight = self.routing(weight).squeeze(1)#b,nhh,d
        
        weight = rearrange(weight, 'b (H n h) d -> b H (n h) d', h = h, H = h, n = n).norm(dim = -1, keepdim = True)
        
        weight = weight/weight.sum(dim = -2, keepdim = True)
        
        if self.train:
        
            weight = F.gumbel_softmax(weight, dim = 1, hard = True) * weight #b,H, nh, 1
            
        else:
            
            weight = weight.ne(weight.data.max(dim = 1, keepdim = True)[0]) * weight
        
        
        k = weight*rearrange(k, 'b (n h) l d -> b l (n h) d', h = h)
        
        
        dots = einsum('b h i d, b h j d -> b h i j', q, k) 
        
        dots = self.grad_mask(dots)
...
class Parallel_Routing_Transformer_layer(nn.Module):
    def __init__(self, iterations, words, dim, depth, heads, dim_head, mlp_dim, dropout = 0.):
        super().__init__()
        self.layers = nn.ModuleList([])
        self.param = nn.Parameter(1e-3*torch.randn(1))
        for _ in range(depth):#L
            self.layers.append(nn.ModuleList([
                
                PreNorm(dim, Parallel_Routing_Attention(iterations = iterations, words = words, dim = dim, heads = int(heads), dim_head = dim_head)),
                PreNorm(dim, FeedForward(dim, mlp_dim, dropout = dropout)),
            ]))
            
        self.Transformer = Transformer(dim, 1, heads, dim_head, mlp_dim, dropout = dropout)
    def forward(self, x):
        for attn_2, ff_2  in self.layers:
           
            x = self.param*attn_2(x, name = 'yuanbeiming', name_didi = 'chendiancheng') + x
            x = self.param*ff_2(x) + x
...
class Parallel_Transformer_layer(nn.Module):
    def __init__(self, words, dim, depth, heads, dim_head, mlp_dim, dropout = 0.):
        super().__init__()
        self.layers = nn.ModuleList([])
        for _ in range(depth):#L
            self.layers.append(nn.ModuleList([
                PreNorm(dim, Attention(dim, heads = heads, dim_head = dim_head, dropout = dropout)),
                PreNorm(dim, FeedForward(dim, mlp_dim, dropout = dropout)),
                PreNorm(dim, Parallel_Attention(words, dim, heads = heads, dim_head = dim_head, dropout = dropout)),
                PreNorm(dim, FeedForward(dim, mlp_dim, dropout = dropout))
            ]))
    def forward(self, x):
        for attn, ff_0, pattn, ff_1 in self.layers:
            x = attn(x, name = 'yuanbeiming', name_didi = 'chendiancheng') + x
            x = ff_0(x) + x
            x = pattn(x, name = 'yuanbeiming', name_didi = 'chendiancheng') + x
            x = ff_1(x) + x
        return x    
...
class Parallel_Transformer(nn.Module):
    def __init__(self, words, dim, depth, heads, dim_head, mlp_dim, dropout = 0.):
        ...
        self.pos_embedding = nn.Parameter(torch.randn(1, words, dim))
        # self.cls_token = nn.Parameter(torch.randn(1, 1, dim))
        self.transformer = Parallel_Transformer_layer(words, dim, depth, heads, dim_head, mlp_dim, dropout = 0.)
        self.dropout = nn.Dropout(dropout)
...
class Parallel_ViT(nn.Module):
    def __init__(self, *, image_size, patch_size,  dim, depth, heads, mlp_dim, channels = 3, dim_head = 64, dropout = 0., emb_dropout = 0.):
        ...
        self.transformer = Parallel_Transformer_layer(num_patches, dim, depth, heads, dim_head, mlp_dim, dropout)
...
class Routing_norm_ViT(nn.Module):
    def __init__(self, *, iterations, image_size, patch_size,  dim, depth, heads, mlp_dim, channels = 3, dim_head = 64, alpha = 0.5, dropout = 0., emb_dropout = 0.):
        #dim: lengh of token ，depth： depth of tranformer, dim_head: dim of signle head, mlp_dim: dim of mlp of transfomer
        super().__init__()
        image_height, image_width = image_size if isinstance(image_size, tuple) or  isinstance(image_size, list) else pair(image_size)
        ...
        # self.transformer = Transformer(dim, depth, heads, dim_head, mlp_dim, dropout)
        self.transformer = Routing_norm_Transformer_layer(iterations, num_patches, dim, depth, heads, dim_head, mlp_dim, alpha, dropout = dropout)
