...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov 27 00:24:22 2023
...
import clip_pgm_perfect_kv_q_cov_doctor_embedding_space as model_vit

import clip_pgm_perfect_kv_q_cov_doctor_II as model_vit

import clip_pgm_perfect_kv_q_cov_master_opposite_plus_ex_II as model_vit
"""
...
#import clip_pgm_perfect_kv_q_cov_master_embedding_space as model_vit 

import Valen_Tine as model_vit
...
#model.load_state_dict(torch.load('/home/user/ybm/pgm/model_Clip_pgm_perfect_cross_cov_master_opposite_II_1200000_neutral_best_99_01.pt', map_location = 'cpu'))
...
#params = torch.load('/home/user/ybm/pgm/model_Clip_pgm_perfect_cross_cov_master_opposite_II_1200000_neutral_best_98_50.pt', map_location = 'cpu')
"""
params = torch.load('/home/user/ybm/pgm/model_against_doctor_II_1200000_neutral_best_98_40.pt', map_location = 'cpu')
for k, q in model.named_parameters():

    if k[:len('discrim')] == 'discrim': 
...
optimiser_G = torch.optim.AdamW(model.module.generate.parameters(), lr=1e-5, weight_decay = weight_decay)

optimiser_D = torch.optim.AdamW(model.module.discrim.parameters(), lr=3e-5, weight_decay = weight_decay)

#optimiser_G.load_state_dict(torch.load('./optimiser_G_'+name+'_now.pt'))
#optimiser_D.load_state_dict(torch.load('./optimiser_D_'+name+'_now.pt'))

#optimiser_D.load_state_dict(torch.load('/home/user/ybm/pgm/optimiser_D_against_doctor_II_1200000_neutral_best_98_49.pt'))

lr_decay_D = torch.optim.lr_scheduler.StepLR(optimiser_D, step_size= 1, gamma= 0.995)
lr_decay_G = torch.optim.lr_scheduler.StepLR(optimiser_G, step_size= 1, gamma= 0.995)



for param_group in optimiser_D.param_groups:
    #param_group["lr"] = 1e-4
    param_group["weight_decay"] = weight_decay
""""""


# lr_decay = torch.optim.lr_scheduler.StepLR(optimiser, step_size= 1, gamma= 0.95)

for param_group in optimiser_G.param_groups:
    #param_group["lr"] = optimiser_D.param_groups[0]["lr"]
    param_group["weight_decay"] = weight_decay
""""""
print('G_lr:', optimiser_G.state_dict()['param_groups'][0]['lr'])
print('G_w_d:', optimiser_G.state_dict()['param_groups'][0]['weight_decay'])


print('D_lr:', optimiser_D.state_dict()['param_groups'][0]['lr'])
print('D_w_d:', optimiser_D.state_dict()['param_groups'][0]['weight_decay'])
...
while epoch < num_epoch:
    ...
    with tqdm(total=len(train_loader)  + len(val_loader)) as pbar:
        ...
        for x_train, label,  label_in, idx in train_loader:
            ...
            if epoch%num_D_iter == 0:
            
                model.module.stop_generate_grad()
    
                out_train = model(x_train) #输出
    
                
    
                loss, right, right_in, choose_right = (model.module.loss_function_D(*out_train, target_shape = label, target_line = label_in, idx = idx) )
                ...
                model.module.open_all_grad()
                #dist.barrier()
                loss = reduce_value(loss, average=True).item()
            ...
            if  epoch%num_G_iter == 0:
                
                model.module.stop_discrim_grad()
            
                out_train = model(x_train) #输出
    
                
    
                loss_g = (model.module.loss_function_G(*out_train, idx= idx) )
                ...
                model.module.open_all_grad()
                #dist.barrier()
                loss_G += reduce_value(loss_g, average=True).item()
                #%%
            ...
            pbar.set_postfix(loss_batch_D = loss.item() if not isinstance(loss, (int, float)) else loss, loss_batch_G = loss_g.item() if not isinstance(loss_g, (int, float)) else loss_g)#进度条
            pbar.update(1)
        ...
        with torch.no_grad():
            ...
            for index, (x_test, label,  label_in, idx)in enumerate(val_loader):
                ...
                _, right, right_in, choose_right = (model.module.loss_function_D(*out_test, target_shape = label, target_line = label_in, idx = idx) )
    ...
    if dist.get_rank() == 0:
        if accuracy_val[2] > max_accuracy: 
            ...
            torch.save(optimiser_G.state_dict(), './optimiser_G_'+name+'_best.pt')
            
            torch.save(optimiser_D.state_dict(), './optimiser_D_'+name+'_best.pt')
    
            max_accuracy = accuracy_val[2]
            
    
        torch.save(model.module.state_dict()  , './model_'+name+'_now.pt')
        torch.save(optimiser_D.state_dict(), './optimiser_D_'+name+'_now.pt')
        torch.save(optimiser_G.state_dict(), './optimiser_G_'+name+'_now.pt')
    ...
    if dist.get_rank() == 0:
        print("epoch:{}\t loss_train:{:.4f}\n accuracy_train: shape:{:.4f}\t line:{:.4f}\t  choose:{:.4f}\n  accuracy_val: shape:{:.4f}\t line:{:.4f}\t choose:{:.4f}\n learning_rate:{:.8f}\n".format(epoch, loss_train,  *accuracy[:3],
                                                                                                                                                                                         *accuracy_val[:3],lr_decay_D.get_lr()[0]))
