...
class Beta(nn.Module):
    def __init__(self, alpha = 10, step_size = 0.01):
        ...
        self.register_buffer("beta",step_size*torch.ones(1))
...
class raven_clip(nn.Module):
    def __init__(self, *args):
        ...
        self.name = 'Clip_pgm_perfect_cross_cov_master_opposite_II'
        ...
        self.temperature_1 = 1e-6
        
        self.temperature = 1e-3

        self.beta = Beta(alpha = 80, step_size = 0.00005)
        ...
        self.txt_clip = nn.Sequential(Rearrange('b n s -> (b n) s', s = 10, n = txt_size),
                            txt_mask_transformer(dict_size = 14, words = 10, dim = self.low_dim*2, depth = num_depth*2, 
                                                 heads = num_head, dim_head = int(self.low_dim*2/num_head), mlp_dim = self.low_dim*2, dropout = 0.1,is_pgm = True),
                            take_cls(),
                            Rearrange('(b n) d -> b n d', n = txt_size)) #b,336,d

        self.num_forward = 0

        self.std_scale = torch.tanh
    ...
    def forward(self, x):
        ...
        qkv = qkv.chunk(self.num_rule, dim = -2) #b s = 16 n = 8 c = 4 m = 2 d
        ...
        return *list(map(lambda t: t.mean(dim = 1).squeeze(), qkv)), x.reshape(-1, self.low_dim), y, sum(list(out))
    ...
    def loss_function_sl(self, *out, target, idx):
        ...
        if keep_rule.float().sum().item() != 0:
            ...
            loss_1 += F.cross_entropy(r.reshape(-1, 7775)/ self.temperature_1, target[keep_rule, None].expand(-1, r.shape[1]).reshape(-1))
    ...
    def loss_function(self, *out, target_shape, target_line, idx):
        ...
        #y_sample = self.reparametrize_times(*y.chunk(2, dim = -1), self.times).permute(1,0,2,3).contiguous()
        y_sample = y.chunk(2, dim = -1)[0].unsqueeze(1)
        """
        ...
        loss_2 = 0
        """
        loss_1, right_shape = self.loss_function_sl(x_shape, y_sample, target = target_shape, idx = idx)
        
        loss_2, right_line = self.loss_function_sl(x_line, y_sample, target = target_line, idx = idx)
        

        """
        right = torch.zeros(1).sum().to(x.device)
        ...
        return 100*loss_1 + 100*loss_2 + 200*loss_3  + loss_4, right_shape, right_line, right# 
    
    def get_mean_std(self, mu, logvar):
        
        return mu, logvar.div(2).exp()      
    
    def reparametrize_times(self, mu, logvar, times):

        std = logvar.div(2).exp()
        
        
        eps = std.data.new(int(times), *std.size()).normal_()
        
        out = mu[None] + self.std_scale(std[None]) * eps 
        return out
